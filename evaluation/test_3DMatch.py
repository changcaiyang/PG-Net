...
def eval_3DMatch(model, config, use_icp):
    ...
    for scene_ind, scene in enumerate(scene_list):
        dset = ThreeDMatchTest(
                               root=f'/media/lixiaohan/下载/huohu/PointDSC-master/data/3DMatch',
...
if __name__ == '__main__':
    ...
    parser.add_argument('--chosen_snapshot', default='PGNet_3DMatch_yw1', type=str, help='snapshot dir')
    ...
    if args.use_icp:
        log_filename = f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{args.chosen_snapshot}-{args.solver}-{config.descriptor}-ICP.log'
    else:
        log_filename = f'/media/lixiaohan/下载/huohu/PointDSC-master/evaluation/logs/{args.chosen_snapshot}-{args.solver}-{config.descriptor}.log'
    ...
    for i in range(15, 51):
        miss = model.load_state_dict(torch.load(f'/media/lixiaohan/下载/huohu/PointDSC-master/snapshot/{args.chosen_snapshot}/models/model_{i}.pkl'), strict=False) #best
        print(miss)
        print(i)
        model.eval()
